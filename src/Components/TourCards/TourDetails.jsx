...
function TourDetails({ addToCart, cartTours, removeFromCart, newTours }) {
  ...
  const ChooseTransportHendler = e => {
    ...
    let transOpt = data.transportOption.find(
      el => el.transport === choosenTransport
    );
  ...
  return (
    <div className="tour-info container">
      ...
      <p className="tour-info-text">
        Departure at {startDay.toLocaleDateString()} from: {data.departureCity}
      </p>
      <p className="tour-info-text">
        Arrival back at {endDay.toLocaleDateString()}
      </p>
      <p className="tour-info-text">
        {(endDay - startDay) / (24 * 60 * 60 * 1000)} days tour
      </p>
      <Swiper {...params} className="swiper tour-info-slider">
        {data &&
          data.arrOfImg.map((img, index) => {
            return (
      ...
      <p className="tour-info-text">
        Tour for {data.numberOfTrevellers} person.
      </p>
      ...
      {!match && (
        <div>
          <p className="tour-info-message">
          To count the price of tour you should choose transport option below:
          </p>
          <div className="tour-info-transport">
            {data &&
              data.transportOption.map(transOpt => {
                return (
                  <label
                    htmlFor={transOpt.transport}
                    key={data.id + transOpt.transport}
                  >
                    {transOpt.transport}
                    <input
                      id={transOpt.transport}
                      type="radio"
                      name="transport"
                      value={transOpt.transport}
                      onChange={e => ChooseTransportHendler(e)}
                    />
                  </label>
                );
              })}
          ...
          {transport.length ? (
            <>
              <p className="tour-info-cost">
                Price of tour: {transport.length && transport[0].price} $.
              </p>
              <button onClick={AddToCartHendler} className="btn-main">
                Book the tour
              </button>
            </>
          ) : null}
      ...
      {match && (
        <div className="tour-info-delete">
          <div>
            <p className="tour-info-delete-text">
              This tour already exist in the cart.
            </p>
            <p className="tour-info-delete-text"> Do you want to delete it? </p>
          </div>
          <svg
            onClick={e => removeFromCart(data.id)}
            aria-hidden="true"
            focusable="false"
            data-prefix="fas"
            data-icon="trash"
