...
function TourItem({
  ...
  const heartDecoration = () => {
    if (liginedUser.length && !liginedUser[0].isAdmin) {
      ...
      fav[1].find(tour => {
        if (tour.id === data.id) {
          return setInFav("svg-inline--fa fa-heart fa-w-16 active-heart");
        }
      });
    }
  };
  ...
  return (
    <div className="col-6 col-md-4 tour-card-wrapper some">
      ...
      <div className="tour-item-back tour-card">
        ...
        <h3>
          <span className="tour-item-departure"> Departure from </span>
          {data.departureCity}
        </h3>
        <h3>
          <span className="tour-item-departure"> Arrival city </span>
          {data.arrivalCity} ({data.arrivalCountry} )
        </h3>
        <p>
          from {startDay.toLocaleDateString()} to {endDay.toLocaleDateString()}(
          {(endDay - startDay) / (24 * 60 * 60 * 1000)} days)
        </p>
        <p> tour for {data.numberOfTrevellers} person</p>
        <p> Price from: {price} $.</p>
        <div className="tour-item-back-more">
          <Link to={{ pathname: `/tour/${data.id}` }}>
            <svg
              aria-hidden="true"
              focusable="false"
              data-prefix="fas"
