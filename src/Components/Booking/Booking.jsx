...
function Booking({ loginUser, cartTours, addOrder }) {
  ...
  const [surname, setSurname] = useState(
    loginUser.length ? loginUser[0].surname : ""
  );
  const [email, setEmail] = useState(
    loginUser.length ? loginUser[0].email : ""
  );
  const [city, setCity] = useState(loginUser.length ? loginUser[0].city : "");
  const [tel, setTel] = useState(
    loginUser.length ? loginUser[0].telefone : +380
  );
  ...
  const finishBookingHendler = () => {
    if (name && surname && email && city && validNumber) {
      let userInfo = loginUser.length
        ? loginUser
        : [
            { name: name, surname: surname, email: email, city: city, tel: tel }
          ];
      let order = [
        [...userInfo],
        [...cartTours],
        ...
        {
          id: `${+new Date()}${userInfo[0].login}`,
          bookingStatus: "consideration"
        }
      ];
      addOrder(order);
